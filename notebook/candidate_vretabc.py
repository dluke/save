...
def compile_fj_reference(idx,  objectives, target):
    ...
    candidate = _fj.lintrackload([idx])[0]
    local = fjanalysis.lsummary([candidate])
    for obs in objectives:
        reference[obs] = make_get(obs)(local)
    return reference

lrefdata = pwlstats.load_candidate_statistics(target)

fj_reference = compile_fj_reference(2924,  objectives, target)
fj_reference
...
params
# %%
params.loc[params['uid'] == "_u_fRlKodZ8"]
...
def compile_objectives(params, ldata, lrefdata):
    ...
    ref = lrefdata["angles"]
    ks_score = lambda x: scipy.stats.ks_2samp(x, ref)[0]
    ks_pwl_length = [ks_score(ld['angles']) for ld in ldata]
    params['ks_angle'] = ks_pwl_length
...
objectives = ["fanjin.candidate.ks_statistic"]
objectives = ['fanjin.candidate.ks_statistic', 'step_rate', 'ks_length', 'deviation.var']
...
obs_setup = [
    ["kmsd.mean"],
    [f"fanjin.candidate.ks_statistic"],
    ["lvel.mean"],
    ["deviation.var"],
    ["qhat.estimate"],
    ["ahat.estimate"],
    ...
    [f"fanjin.candidate.ks_statistic", "deviation.var", "ks_length"],
    [f"fanjin.candidate.ks_statistic", "deviation.var", "qhat.estimate", "ahat.estimate"]
]
...
# pub.save_dfhtml(qdf, "sim_t7_vretabc", notename)
qdf
...
sns.histplot(candidate_lvel, ax=ax, stat='density', element="step", fill=False, alpha=0.8)
# ax.legend(['t6', 'candidate'])
ax.legend([best_uid[3:], 'candidate'])

# %%
ks_stat, pv = scipy.stats.ks_2samp(lvel, candidate_lvel)
ks_stat
