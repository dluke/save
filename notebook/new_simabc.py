...
if True:
    fig, axes = abcimplement.perfectplot4d(
        sim4d["problem"], _accepted, simpar=simpar, mpar=m_par)
...
fig, ax = plt.subplots(figsize=(5, 5))
sns.scatterplot(data=_accepted, x="anchor_angle_smoothing_fraction", y="k_spawn")
...
def plot_alpha_pdisp(ax):
    sns.scatterplot(data=accparams, x="anchor_angle_smoothing_fraction", y="pdisp", ax=ax)
    ax.set_ylabel(r"$\langle \Delta x \rangle$", fontsize=20)
    ax.set_xlabel(r"$\alpha$", fontsize=20, labelpad=0, backgroundcolor="white")
...
with mpl.rc_context({"font.size": 20, "text.usetex":True}):
    ...
    # inset.set_xlim((0,np.pi/2))
    # why does mpl hide labesls when argument is empty list []?
    x = accparams["anchor_angle_smoothing_fraction"]
