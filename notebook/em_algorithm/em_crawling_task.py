...
data_dir = join('./', 'crawling_em_quad_4/')
...
def preprocess(data, q=0.005, delta=0.005):
	...
	data = emanalyse.symprocess(data, q, remove_zeros=remove_zeros)
	return data
...
def fixed_process(data, xlim=(-0.05, 0.10), delta=0.005, remove_zeros=True):
	...
	#! seems we actually forgot to remove zeros ... and results still made sense
	return data[keep]
...
delta = 0.005
xlim = (-0.10, 0.10)
datalist = [fixed_process(udata, xlim, delta, remove_zeros=remove_zeros) for udata in udatalist]
...
def g_emg_constraint(sigma=_sigma, lam=_lam):
	...
	construct.set_numerical_instructions(pack, unpack)
	l1_max = 500
	construct.set_bounds([[0, None], [0, None], [0, l1_max]])
	return construct
...
def N_NTN(par=[0.02, 0.009, 0.09]):
	construct = MixtureModel(par[1], 
		numerical=True, n_constraints=1, err='normal')
	construct.add_impulse(0, fix_loc=True)
	construct.add_truncated_normal(par , err='normal')
	def pack(mix):
		# s is the error distribution paramter
		# sigma is the truncated normal
...
def L_LTN(par=[0.00, 0.09, 1/0.013]):
	construct = MixtureModel(_sigma, 
...
model_names = ['g_emg', 'l_eml', 'N_NTN', 'L_LTN']
model_list = [g_emg_constraint, L_eml, N_NTN, L_LTN]
