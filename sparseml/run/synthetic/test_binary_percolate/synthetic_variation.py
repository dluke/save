...
def run_variation(job_index, target_dir, pwl, params, update, control={}):
    ...
    loss_conf = {"contour_term": 0.05}
    local_config['loss_conf'] = loss_conf
    
    # construct solver
    partition = pwlpartition.PartAnneal(wavemodel, lptr, loss_conf=loss_conf, inter_term=False)
    rng = np.random.RandomState(seed)
    solver = pwlpartition.Solver(partition, rng=rng, r=r, sigma=meta["sigma"], 
            min_constraint=1, use_binary_percolate=True)
