...
# generate 
N = 40
params = {"l": 1.0, "sigma": 0.05, "N": 40, "dx": 0.1, "p_threshold": 0.98, "angle_sigma": pi/8}


def save_angle_distribution(target_dir, params):
    target = join(target_dir, 'angle_distribution.png')
    angle_sigma = params.get("angle_sigma")
    angle = synthetic.Normal(loc=0, scale=angle_sigma)
    with mpl.rc_context({'font.size': 24}):
        fig, ax = plt.subplots()
        synthetic.exactplot(ax, angle)
        plt.tight_layout()
        fig.savefig(target)
...
def run_variation(job_index, target_dir, params, update, control={}):
    ...
    save_angle_distribution(target_dir, local_params)
    ...
    solver.dump_state(join(target_dir, 'initial_solver'))
    ...
    # the initial guess needs some work
    with support.PerfTimer() as timer:
        solver.linear_solve()
        solver.percolate()
        solver.percolate()
    initial_exec_time = timer.get_time()

    output = pwlpartition.Output(target_dir, allow_write=True, index=job_index)
    with support.PerfTimer() as timer:
...
output_form = "{}_{:07.4f}"
pi = np.pi
variant_basis = [pi/4, pi/6, pi/8, pi/10, pi/12, pi/14, pi/16]
...
for job_id, update in enumerate(update_list):
    ...
    p = multiprocessing.Process(target=run_variation, args=[job_id, target_dir, params, update])
    p.start()
