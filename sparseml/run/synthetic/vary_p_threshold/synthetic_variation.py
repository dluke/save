...
# generate 
params = {"l": 1.0, "sigma": 0.03, "N": 20, "dx": 0.1, "p_threshold": 0.98, "angle_sigma": pi/8}
...
def generate_pwl(params):
    ...
    pwl = new_static_process(length, mirror_angle, N)
    return pwl
...
def save_local_synthdata(target_dir, synthdata):
...
def run_variation(job_index, target_dir, pwl, params, update, control={}):
    ...
    save_local_synthdata(target_dir, synthdata)
...
variant = 'sigma'
output_form = "{}_{:07.4f}"
variant_basis = [0.03,0.05,0.10,0.15, 0.20,0.30,0.40]
# variant_basis = [0.40]
